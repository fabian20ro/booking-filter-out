(function () {
    ...
    function updateStatus() {
        ...
        if (el) el.textContent = core.getSavedList().length + ' saved';
    }
    ...
    var buttons = [
        ...
        ['Copy all saved', '\uD83D\uDCCB', function () { 
            ...
            copyTextList(saved, 'No hotels to copy.', function (count) { return 'Copied ' + count + ' hotel names.'; });
        }],
        ['Copy non-excluded', '\uD83D\uDCCB', function () { 
            var nonExcluded = core.getNonExcludedVisibleHotels();
            if (!nonExcluded.length) { showMessage('No non-excluded hotels to copy.'); return; }
            copyTextList(nonExcluded, 'No non-excluded hotels to copy.', function (count) { return 'Copied ' + count + ' hotel names to clipboard.'; });
        }],
        ['Clear list', '\uD83E\uDDF9', function () { 
